...
TEST(StandardScalerTest, ScaleMatrix)
{
    sk::Tensor a = sk::tensor::arange(4);
    a += 1;
    a.reshape({ 2, 2 });

    sk::scaler::StandardScaler scaler{ a };
    ...
    EXPECT_EQ(scaled(0, 0), -1);
    EXPECT_EQ(scaled(0, 1), -1);
    EXPECT_EQ(scaled(1, 0), 1);
    EXPECT_EQ(scaled(1, 1), 1);
}

TEST(StandardScalerTest, ScaleUnscaleMatrix)
{
    sk::Tensor a = sk::tensor::arange(4);
    a += 1;
    a.reshape({ 2, 2 });
